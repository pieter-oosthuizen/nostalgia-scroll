...
def _infer_date_order(lines: list[str], *, sample_size: int = 500) -> str:
    """
    Infer whether the export is DMY or MDY.

    WhatsApp exports often use ambiguous dates like 01/02/2020. When ambiguous,
    we prefer DMY (common outside the US) to avoid "messages from the future".
    """
    dmy = 0
    mdy = 0
    seen = 0
